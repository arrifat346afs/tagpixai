...
const GenerateButton = () => {
  const { selectedFiles, setSelectedFile, setSelectedFileMetadata } = useContext(FileContext)
  ...
  const handleFileComplete = async (result: ProcessingResult) => {
    ...
    if (result.success && result.metadata) {
      try {
        ...
        const isIncomplete = !result.metadata.title ||
        ...
        setTimeout(() => {
          console.log(`Setting selected file to ${result.filePath} (AI selection)`);
        ...
        if (isIncomplete) {
          toast.warning(`Incomplete metadata for ${fileName}`, {
            description: "Some metadata fields are missing or empty"
          });
          updateStatus(`Partially processed ${fileName} - some fields are missing`)
        } else {
      ...
      try {
        ...
        setTimeout(() => {
          console.log(`Setting selected file (failed) to ${result.filePath} (AI selection)`);
